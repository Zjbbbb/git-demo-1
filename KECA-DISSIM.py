...
def Slide_Keca(K, k, n, L, gamma, n_components, err):
...
def Dissim(X1, X2):
    ...
    X_cov = N1/(N1+N2) * X1_cov + N2/(N1+N2) * X2_cov

    eigenvalues, eigenvectors = np.linalg.eig(X_cov)
    eigenvalues_ = np.power(eigenvalues, 0.5)
    eigenvalues__ = np.reciprocal(eigenvalues_)
    eigenvalues_diag = np.diag(eigenvalues__)

    P = eigenvectors @ eigenvalues_diag

    Y1 = math.sqrt(N1/(N1+N2)) * X1 @ P
    S1 = 1/N1 * Y1.T @ Y1
    eigenvalues_S1 = np.linalg.eig(S1)[0]

    m1 = len(eigenvalues_S1)
    D1 = 0
    for i in range(m1):
        D1 = D1 + (eigenvalues_S1[i]-0.5) ** 2
    D1 = D1 * 4 / m1
...
if __name__ == '__main__':
    ...
    X_train_Slide_KECA = Slide_Keca(
        K, k, n1, L, gamma, n_components, 0)

    # 计算训练集各个批次之间的Dissim相异度 Distances_train 是 n1*n1 的矩阵
    Distances_train = np.zeros((n1, n1))
    for i in range(n1):
        for j in range(i+1, n1):
            Distances_temp = Dissim(X_train_Slide_KECA[(K-L+1)*i:((K-L+1)*(i+1)),],
                                    X_train_Slide_KECA[(K-L+1)*j:((K-L+1)*(j+1)),])
            Distances_train[i][j] = Distances_temp
            Distances_train[j][i] = Distances_temp
    ...
    Boundary_C_B = Dissim(Boundary_vector, Center_vector)

    # 计算中心批次和测试批次之间的KECA-DISSIM指标
    X_test1_Slide_KECA = Slide_Keca(K, k, n2, L, gamma, n_components, noise1)
    Distances_test1 = np.zeros(n2)
    for i in range(n2):
        Distances_test1[i] = Dissim(X_test1_Slide_KECA[(K-L+1)*i:((K-L+1)*(i+1)),],
                                    Center_vector)

    X_test2_Slide_KECA = Slide_Keca(K, k, n2, L, gamma, n_components, noise2)
    Distances_test2 = np.zeros(n2)
    for i in range(n2):
        Distances_test2[i] = Dissim(X_test2_Slide_KECA[(K-L+1)*i:((K-L+1)*(i+1)),],
                                    Center_vector)

    End_time = time.time()
    Run_time = End_time - Start_time
    print("Run_time:", '%.2f' % Run_time, 's')

    Boundary_label = '{:.3f}'.format(Boundary_C_B)
    fig, ax = plt.subplots(nrows=1, ncols=2, figsize=(12, 5))
    ax[0].plot(range(n2), Distances_test1, color='black')
    ax[0].axhline(y=Boundary_C_B, color='red', linestyle='--')
    ax[0].text(0, Boundary_C_B, Boundary_label, ha='right', va='center')
    ax[0].set_xlabel('Index')
    ax[0].set_ylabel('Distances')
    ax[0].set_xlim(0, n2)
    ax[0].set_ylim(0, None)
    ax[0].set_title('Test1_{}'.format(noise1))
    ax[1].plot(range(n2), Distances_test2, color='black')
    ax[1].axhline(y=Boundary_C_B, color='red', linestyle='--')
    ax[1].text(0, Boundary_C_B, Boundary_label, ha='right', va='center')
    ax[1].set_xlabel('Index')
    ax[1].set_ylabel('Distances')
    ax[1].set_xlim(0, n2)
    ax[1].set_ylim(0, None)
    ax[1].set_title('Test2_{}'.format(noise2))
